...
def _synthesize_raw(tts_engine, text: str, wav_path: str,
                    speaker_wav: str | None = None) -> bytes | None:
...
def text_file_to_speech(source_path, output_path, tts_engine=None, *,
                        alignment=None, speaker_wav=None, voice_map=None):
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, seg_speaker: str | None = None) -> tuple[int, bytes | None]:
            ...
            return idx, _synthesize_raw(engine, text, wav_path, speaker_wav=effective_wav)
